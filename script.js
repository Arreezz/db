document.addEventListener('DOMContentLoaded', () => {
    ...
    /**
     * Ensures a URL is safe to use in an href by prepending http:// if no protocol is present.
     * @param {string} url The URL to process.
     * @returns {string} A URL with a protocol.
     */
    ...
    function attachRowDetailsTo(tbodyId, titlePrefix) {
        ...
        tb.addEventListener('click', (e) => {
            ...
            const cells = Array.from(tr.children).map(td => td.innerHTML); // Use innerHTML to preserve links
    ...
    function renderRotwTable(searchTerm = '') {
        ...
        filteredRows.forEach(({ row, originalIndex }) => {
            ...
            const productLinkHTML = row.productLink ? 'LINK' : '';
            tr.innerHTML = `
                ...
                <td class="table-cell text-center">${productLinkHTML}</td>
                ...
                <td class="table-cell">${row.date || ''}</td>
                <td class="table-cell text-center space-x-1">
                    <button class="rotw-open-link-btn px-2 py-1 bg-indigo-600 text-white rounded hover:bg-indigo-500" data-index="${originalIndex}" title="Open Product Link" ${!row.productLink ? 'disabled' : ''}><i class="fas fa-up-right-from-square"></i></button>
    ...
    if (rotwTableBody) {
        rotwTableBody.addEventListener('click', (e) => {
            ...
            if (button.classList.contains('rotw-open-link-btn')) {
                const link = rotwStore[index].productLink;
                if (link) {
                    window.open(getSafeUrl(link), '_blank');
                }
            } else if (button.classList.contains('rotw-edit-btn')) {
            ...
            .forEach(({ row, idx }) => {
                ...
                tr.innerHTML = `
                    ...
                    <td class="table-cell text-center space-x-2">
                        <button class="china-open-link-btn px-2 py-1 bg-indigo-600 text-white rounded hover:bg-indigo-500" title="Open Product Link" ${!row.productLink ? 'disabled' : ''}><i class="fas fa-up-right-from-square"></i></button>
        ...
        const rows = draftsStore
            ...
            .map(({ row, idx }) => `
                <tr data-index="${idx}">
                    ...
                    <td class="table-cell">${row.date || ''}</td>
                    <td class="table-cell text-center space-x-1">
                        <button class="draft-print-btn px-2 py-1 bg-sky-600 text-white rounded hover:bg-sky-500" title="Print Draft"><i class="fas fa-print"></i></button>
    ...
    function buildDraftDetailsHTML(data = {}) {
        ...
        chinaFields.forEach((key, idx) => {
            ...
            if (key === 'productLink' && value) {
                value = 'LINK';
            }
            rows += `<tr><th class="text-left pr-4 py-1 text-gray-300">${label}</th><td class="py-1 text-gray-100">${value}</td></tr>`;
        });
        return `<div class="overflow-auto"><table class="w-full">${rows}</table></div>`;
    }
    ...
    if (draftsModalBody) {
        draftsModalBody.addEventListener('click', (e) => {
            ...
            if (btn.classList.contains('draft-print-btn')) {
                // NOTE: This functionality requires a corresponding '.printable-china-container' div in index.html and print styles in style.css.
                const draftData = draftsStore[idx];
                if (!draftData) {
                    showNotification('Could not find draft data to print.', 'error');
                    return;
                }

                const chinaPrinter = document.querySelector('.printable-china-container');
                if (!chinaPrinter) {
                    showNotification('Print template not found in HTML.', 'error');
                    return;
                }

                // Populate the basic info
                const customerNameEl = chinaPrinter.querySelector('#print-china-customerName');
                const trackNoEl = chinaPrinter.querySelector('#print-china-trackNo');
                const dateEl = chinaPrinter.querySelector('#print-china-date');
                if(customerNameEl) customerNameEl.textContent = draftData.customerName || '';
                if(trackNoEl) trackNoEl.textContent = draftData.trackNo || '';
                if(dateEl) dateEl.textContent = draftData.date || '';

                // Populate the details table
                const detailsTable = chinaPrinter.querySelector('.print-main-content-china');
                if (detailsTable) {
                    let tableHtml = '<tbody>';
                    const dataInPairs = [];
                    chinaFields.forEach((key, index) => {
                        if (['customerName', 'trackNo', 'date'].includes(key)) return;
                        const label = chinaLabels[index];
                        const value = draftData[key] || '';
                        if (value) {
                            dataInPairs.push({ label, value });
                        }
                    });

                    for (let i = 0; i < dataInPairs.length; i += 2) {
                        const item1 = dataInPairs[i];
                        const item2 = (i + 1 < dataInPairs.length) ? dataInPairs[i + 1] : null;
                        tableHtml += '<tr>';
                        tableHtml += `<th>${item1.label}</th><td>${item1.value}</td>`;
                        if (item2) {
                            tableHtml += `<th>${item2.label}</th><td>${item2.value}</td>`;
                        } else {
                ...
                const receiptPrinter = document.querySelector('.printable-receipt-container');
                if (receiptPrinter) receiptPrinter.style.display = 'none';
                chinaPrinter.style.display = 'block';

                window.print();

                chinaPrinter.style.display = 'none';
                return;
            }
